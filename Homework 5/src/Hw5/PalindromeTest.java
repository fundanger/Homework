...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
    	if (s == null) 
    	{
    		throw new IllegalArgumentException(); //throw exception if the string is null
    	}
    	String newString = s.replaceAll("\s+","").toLowerCase(); //replace whitespace and make string lowercase
    	ArrayListStack<Character> palindrome = new ArrayListStack<Character>();
    	char[] stringToChar = newString.toCharArray();
    	if(newString.length() == 1) // Single characters are palindromes 
    	{
    		return true;
    	}    	
    	for (int i = 0; i < newString.length() /2; i++) //pushing half of the new string to the ALS Palindrome
    		{    		
    		palindrome.push(stringToChar[i]);
    		}
    	for (int i = (newString.length() + 1) / 2; i < newString.length(); i++) //Comparing half of the palindrome to the other half
    	{    		
    		if (palindrome.pop() != stringToChar[i])
    		{
    			return false;
    		}
    	}
